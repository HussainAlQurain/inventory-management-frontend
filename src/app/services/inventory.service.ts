...
export class InventoryService {
  ...
  getItemsByLocation(locationId: number): Observable<InventoryItem[]> {
    return this.http.get<InventoryItem[]>(`${this.apiUrl}/locations/${locationId}/items`);
  }
  
  // Get all available units of measure for counting
  getAvailableUoms(): Observable<any[]> {
    return this.http.get<any[]>(`${environment.apiUrl}/uoms`);
  }
}
